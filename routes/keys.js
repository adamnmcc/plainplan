...
function createKeysRouter(pool) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      res.status(201).json({ success: true, api_key: key, key_prefix: keyPrefix, message: 'Store this key securely — it cannot be retrieved later.', dashboard_url: `https://dev.api.plainplan.click/dashboard?key=${key}` });
    } catch (err) {
