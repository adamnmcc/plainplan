...
FIXTURE_PATH = BASE_DIR / "test-fixtures" / "sample-plan.json"
...
def create_key(payload: dict[str, Any]) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        content={
            ...
            "dashboard_url": f"https://planplain.polsia.app/dashboard?key={raw_key}",
        },
    )
...
async def legacy_analyze(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(plan.get("resource_changes"), list):
        raise HTTPException(status_code=400, detail={"error": "Invalid Terraform plan JSON. Expected resource_changes array."})
    return {
        "summary": {"total_changes": len(plan["resource_changes"])},
        "risk_flags": [],
        "reviewer_checklist": ["Review full plan output"],
...
def home_page() -> HTMLResponse:
    ...
    if not index_path.exists():
        return HTMLResponse("<pre>{\"message\": \"PlanPlain API\"}</pre>")

    html = index_path.read_text(encoding="utf-8")
    html = html.replace("__POLSIA_SLUG__", get_settings().polsia_analytics_slug)
